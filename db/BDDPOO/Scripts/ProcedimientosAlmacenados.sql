...
ELSE
        ...
        SET nivelAcceso = (SELECT sp_obtenerNivelDeAccesoDeEmpleado(sp_idEmpleado));

        /* Verificar si el nivel de acceso permite crear casos sin adjunto */
        IF nivelAcceso = 'Jefes de áreas funcionales' OR nivelAcceso = 'Administrador' THEN
    ...
    /* Si hay errores, establecer el mensaje de error */
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM bitacorarequerimiento WHERE PK_idBitacoraRequerimiento = sp_idBitacoraRequerimiento) THEN
...
WHERE PK_idBitacoraRequerimiento = sp_idBitacoraRequerimiento;
...
BEGIN
    ...
    IF NOT EXISTS (SELECT * FROM casorequerimiento WHERE PK_idCasoRequerimiento = sp_idCasoRequerimiento) THEN
...
DELIMITER ;
   ...
   /*Insertar casos (estos son necesarios para la hora de registrar sus avances en la bitacora*/
DELIMITER //

CREATE PROCEDURE sp_InsertarCasoRequerimiento(
    IN sp_TituloCasoRequerimiento VARCHAR(250),
    IN sp_porcentajeAvance INT,
    IN sp_FK_idEstadoRequerimiento INT,
    IN sp_FK_idEmpleado INT,
    IN sp_FK_idEmpleadoProbador INT,
    IN sp_FK_idProyecto INT,
    OUT mensaje VARCHAR(255)
)
BEGIN
    DECLARE error_occurred BOOLEAN DEFAULT FALSE;
    ...
    IF sp_TituloCasoRequerimiento = '' OR sp_FK_idEstadoRequerimiento IS NULL OR sp_FK_idEmpleado IS NULL OR sp_FK_idProyecto IS NULL THEN
        SET mensaje = 'Uno o más parámetros obligatorios están vacíos.';
        SET error_occurred = TRUE;
ELSE
        /* Insertar el nuevo caso de requerimiento */
BEGIN
INSERT INTO CasoRequerimiento (TituloCasoRequerimiento, porcentajeAvance, FK_idEstadoRequerimiento, FK_idEmpleado, FK_idEmpleadoProbador, FK_idProyecto)
VALUES (sp_TituloCasoRequerimiento, sp_porcentajeAvance, sp_FK_idEstadoRequerimiento, sp_FK_idEmpleado, sp_FK_idEmpleadoProbador, sp_FK_idProyecto);

/* Asumir que no hubo errores y establecer mensaje de éxito */
SET mensaje = 'Caso de requerimiento insertado correctamente.';
END;
END IF;

    /* Si hay errores, establecer mensaje de error */
    IF error_occurred THEN
        SET mensaje = 'Error al insertar el caso de requerimiento.';
END IF;
END//
