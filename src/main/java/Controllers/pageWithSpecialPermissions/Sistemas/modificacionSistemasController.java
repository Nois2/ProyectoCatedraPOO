...
@WebServlet(name = "modificacionSistemasController", value = "/modificacionSistemasController")

public class modificacionSistemasController extends HttpServlet {
    public void init() {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (session.getAttribute("datosEmpleado") != null) {
            ...
            if (emp.getFK_idNivelDeAcceso() == rolAdministrador) {
                RequestDispatcher rd = request.getRequestDispatcher("WEB-INF/pageWithSpecialPermissions/Sistemas/Funcionalidades/modificacionDeFuncionalidad.jsp");
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        HttpSession session = request.getSession();
        try {
            Empleados e = (Empleados) session.getAttribute("datosEmpleado");
            if(e.getFK_idNivelDeAcceso() == 1) {
